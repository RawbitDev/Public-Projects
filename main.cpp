// Praktikum 3 Exercise 3 von Ricardo Cincato (760994), Ramon Walther (761312) und Celine Heymel (761360) -> Gruppe:Mo34y-8
#include <iostream>
#include <string>
using namespace std;
...
{  
    ...
    while (true) {
        ...
        if (input == "exit") {
            ...
            cerr << "Fehler: Ungueltige Eingabe! Bitte versuchen Sie es erneut." << endl << endl;
        }
    }
...
int MakeMove(string input) {
    ...
    char OldColumn = '0', NewColumn = '0', OldRow = '0', NewRow = '0';

    // Splits input to chars
    OldColumn = input.at(0);
    OldRow = input.at(1);
    NewColumn = input.at(3);
    NewRow = input.at(4);

    // Checks if the selected flieds are correct
    if (((ReadField(OldColumn, OldRow)) != (FillString("", "_"))) && ((ReadField(NewColumn, NewRow)) == (FillString("", "_")))) {
        ...
        cerr << "Fehler: Ungueltige Eingabe der Felder! Bitte versuchen Sie es erneut." << endl << endl;
    }
...
// Function that check if input is correct (Error handling)
bool CheckInput(string input)
{
    // Check if input length is right
    if (input.length() == 5) {

        int OldColumn = '0', NewColumn = '0', OldRow = '0', NewRow = '0';
        ...
        if ((OldColumn>=97 && OldColumn<=104) && (OldRow>=49 && OldRow<=56) && (NewColumn>=97 && NewColumn<=104) && (NewRow>=49 && NewRow<=56)) {
            return true;
        }
